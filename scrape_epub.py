...
def set_attribute(browser, el, attr, val):
    if attr not in ["innerText", "outerText", "innerHTML", "outerHTML"]:
        browser.execute_script(f"arguments[0].setAttribute(`{attr}`,`{val}`)",
                               el)
    else:
...
def get_attribute(browser, el, attr):
    if attr not in ["innerHTML", "outerHTML", "innerText", "outerText"]:
        ...
        d = bs4.BeautifulSoup(el.get_attribute("outerHTML"),
                              parser).select_one(tn)
        # print(d)
        return d[attr]
    else:
...
nthu_home = str(requests.get("https://web.ntnu.edu.tw/~algo/",
                             headers={
...
for link in nthu_home.select("a"):  #[147:154]:
    myLink = link["href"]
    myLink = links(myLink, "https://web.ntnu.edu.tw/",
                   "https://web.ntnu.edu.tw/~algo/")
    myPage = str(requests.get(myLink, headers={
        "Accept-Encoding": "UTF-8"
    }).content,
                 encoding="UTF-8")
    ...
    with open("temp.html", "w", encoding="UTF-8") as ff:
        ff.write(myPage)
    browser.get("file://" + ("/" if os.name == "nt" else "") +
                os.getcwd().replace("\\", "/") + "/temp.html")
    browser.execute_script("document.querySelectorAll('div.m').forEach(d=>d.parentElement.removeChild(d));")
    folderName = browser.find_element(By.CSS_SELECTOR,
                                      "title").get_attribute("innerText")[:-8]
    folderName = "".join(
        ch for ch in folderName
        if (ch.isalnum() or re.search(u'[\u4e00-\u9fff]', ch)))
        ...
        for mySrc in browser.find_elements(By.CSS_SELECTOR, src_item):
            try:
                ...
                if len(x) == 0:
                    abc = 0
                    abc /= 0
                x = links(x, "https://web.ntnu.edu.tw/",
                          "https://web.ntnu.edu.tw/~algo/")
                ...
                def k(links, x):
                    ...
                    try:
                        ...
                        with open(y, "wb") as f:
                            f.write(
                                requests.get(x, headers={
                                    "User-Agent": ua
                                }).content)
                    except Exception as e:
                        print("Some details: ", e)
                        print(
                            "something went wrong! - there was no src code probably"
                        )

                threading.Thread(target=k, args=[links, x]).start()
                set_attribute(
                    browser, mySrc, "src",
                    (get_attribute(browser, mySrc, "src")
                     if x == get_attribute(browser, mySrc, "src") else "" +
                     get_attribute(browser, mySrc, "src").split("/")[-1]))
                time.sleep(0.05)
            except Exception as e:
                print("details: ", e)
                print("some issue")
    for mySrc in browser.find_elements(By.CSS_SELECTOR, "link"):
        try:
            ...
            set_attribute(
                browser, mySrc, "href",
                (get_attribute(browser, mySrc, "href") if x == get_attribute(
                    browser, mySrc, "href") else "" +
                 get_attribute(browser, mySrc, "href").split("/")[-1]))
            time.sleep(0.05)
        except Exception as e:
            print("details: ", e)
            print("some issue")
    time.sleep(WAIT_FOR_ME)
    myFile = folderName + ".html"
    if os.path.exists(myFile):
        x = 2
        myFile = f"{folderName}_{x}.html"
